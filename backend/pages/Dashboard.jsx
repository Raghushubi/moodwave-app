...
export default function Dashboard() {
  ...
  // Fetch songs whenever moodId changes
  useEffect(() => {
    if (!moodId) return;
    fetchMusic(moodId);
  }, [moodId]);

  // Fetch matches ONLY when songs are available and moodId exists
  useEffect(() => {
    if (!moodId || songs.length === 0) return;
    axios
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {/* Social Matching Section – only appear if songs exist */}
      {songs.length > 0 && (
